...
export class OrdersComponent implements OnInit {
  ...
  ngOnInit() {
    this.dataService.getOrders().subscribe((orders: Order[]) => {
      this.orders = orders;
      // Sort orders by dateCreated in descending order (most recent first)
      this.filteredOrders = this.sortOrdersByDateDescending(orders);
    });
  }

  // Sort orders by date created in descending order
  private sortOrdersByDateDescending(orders: Order[]): Order[] {
  ...
  filterOrders() {
    const filtered = this.orders.filter(order => {
      const matchesSearch = order.orderNumber.toLowerCase().includes(this.searchTerm.toLowerCase()) ||
                           order.businessPartner.toLowerCase().includes(this.searchTerm.toLowerCase()) ||
                           order.orderedBy.toLowerCase().includes(this.searchTerm.toLowerCase());
                           order.orderedBy.toLowerCase().includes(this.searchTerm.toLowerCase());
    ...
    this.filteredOrders = this.sortOrdersByDateDescending(filtered);
  }
  ...
  clearFilters() {
    ...
    this.endDate = '';
    this.statusFilter = '';
    this.filteredOrders = this.sortOrdersByDateDescending(this.orders);
  }
